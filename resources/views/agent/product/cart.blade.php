                                    ...
                                    <tbody class="text-center">
                                        ...
                                        @foreach ($globalCart as $item)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <img src="{{ uploaded_asset($item['image']) }}" width="60" class="rounded border" alt="">
                                                </td>

                                                <td class="fw-semibold">₹{{ number_format($item['price'], 2) }}</td>

                                                <td style="width: 160px;">
                                                    <form action="{{ route('agent.cart.update', $item['product_id']) }}" method="POST" class="update-qty-form d-flex justify-content-center">
                                                        @csrf
                                                        <button type="button" class="btn btn-outline-secondary btn-sm qty-minus">−</button>
                                                        <input type="text" name="quantity" value="{{ $item['quantity'] }}" class="form-control text-center mx-2" style="width: 50px;">
